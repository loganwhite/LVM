...
/* Add the lvm toolset header. Modified by Logan. */
#include "lvmcreate.h"
...
/* Create a variable to store the encrypted device number. Modified by Logan. */
static unsigned int crypto_dev_no;
...
static int create_crypto_blk_dev(struct crypt_mnt_ftr *crypt_ftr,
  ...
  /* Get the encrypted device number. Modified by Logan. */
  crypto_dev_no = minor;
...
static int cryptfs_restart_internal(int restart_main)
{
    ...
    if (! (rc = wait_and_unmount(DATA_MNT_POINT, true)) ) {
        ...
        /* Mount the lvm partition. Modified by Logan. */
        if (mount("/dev/mapper/vg-outter",DATA_MNT_POINT, "ext4", 0, NULL) == -1) {
            SLOGE("error mountting outter, %s", strerror(errno));
                            ...
                            int no_ui)
{
  ...
  int index = 0;
    ...
    /* Print the crypto device number to log, Modified by Logan. */
    SLOGI("Crypto device number is %u", crypto_dev_no);
   
    /* Creating LVM devices, Modified by Logan. */
    if (create_lvm(crypto_dev_no)) {
        SLOGE("Error creating lvm devices, Modified by Logan\n");
        goto normal_encrypt;
    }
    SLOGI("Successfully created LVM devices, Modified by Logan\n");

    /* Close LVM devices, Modified by Logan. */
    if (close_lvm()) {
        SLOGE("Error closing LVM devices, Modified by Logan\n");
        goto normal_encrypt;
    }
    SLOGI("Successfully closed LVM devices, Modified by Logan\n");
